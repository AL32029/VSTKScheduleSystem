...
def cabinet_orm_to_domain(
    cabinet: "CabinetORM", check_redirect: bool = False
) -> "Cabinet":
...
def lesson_orm_to_domain(lesson: "LessonORM", check_redirect: bool = False) -> "Lesson":
    ...
    return Lesson(
        ...
        name=lesson.name,
        cabinets=tuple(
            cabinet_orm_to_domain(cab.redirected if check_redirect else cab)
            for _, cab in cabinets
        ),
    )


def lesson_domain_in_orm(
    date: datetime.date, group: "Group", lesson: "Lesson"
) -> "LessonORM":
    return LessonORM(
...
def lessons_orm_to_day_schedule_domain(
    ...
    if len(groups_found) > 1:
        raise ScheduleForSomeGroupsError(
            f"The lessons list contains pairs for different "
            f"groups: {', '.join(str(group) for group in groups_found)}"
        )

    if len(schedule_dates) > 1:
        raise ScheduleForSomeDatesError(
            f"The lessons list contains pairs for different dates: "
