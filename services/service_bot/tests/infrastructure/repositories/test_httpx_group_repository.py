...
async def test_get_by_number(httpx_mock, client, httpx_group_repository, group):
    httpx_mock.add_response(
        ...
        json={"success": True, "data": asdict(group)},
    )
...
async def test_get_all(httpx_mock, client, httpx_group_repository):
    httpx_mock.add_response(
        ...
        json={
            "success": True,
            "data": [
                asdict(group) for group in sorted(_GROUP_ITEMS, key=lambda x: x.index)
            ],
        },
    )
    ...
    assert all(
        isinstance(group, Group) for group in sorted(group_items, key=lambda x: x.index)
    )
