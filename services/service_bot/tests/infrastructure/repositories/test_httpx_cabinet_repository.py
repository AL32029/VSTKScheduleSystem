...
@pytest.mark.parametrize("cabinet", _CABINET_ITEMS)
async def test_get_by_number_not_found(httpx_mock, client, httpx_cabinet_repository, cabinet):
...
async def test_get_all(httpx_mock, client, httpx_cabinet_repository):
    ...
    assert all(isinstance(cabinet, Cabinet) for cabinet in sorted(cabinet_items, key=lambda x: x.index))
