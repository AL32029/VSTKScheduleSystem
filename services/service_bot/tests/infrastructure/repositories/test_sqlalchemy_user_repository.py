...
async def test_user_get_by_id(sqlalchemy_user_repo, saved_user):
...
async def test_user_get_by_id_without_metadata(sqlalchemy_user_repo, saved_user_without_metadata):
...
async def test_user_update_metadata(sqlalchemy_user_repo, saved_user):
    ...
    await sqlalchemy_user_repo.update_metadata(saved_user, 'message_panel_id', new_value)
...
async def test_user_subscribe_group_error_already_inserted(sqlalchemy_user_repo, saved_user_with_subscribed_group):
...
async def test_user_subscribe_cabinet_error_already_inserted(sqlalchemy_user_repo, saved_user_with_subscribed_cabinet):
...
async def test_user_unsubscribe_group(sqlalchemy_user_repo, saved_user_with_subscribed_group):
    await sqlalchemy_user_repo.unsubscribe_group(saved_user_with_subscribed_group, _GROUP_ITEM.index)

    assert _GROUP_ITEM.index not in saved_user_with_subscribed_group.group_subscribes


async def test_user_unsubscribe_group_error_subscribe_not_found(sqlalchemy_user_repo, saved_user):
...
async def test_user_unsubscribe_cabinet(sqlalchemy_user_repo, saved_user_with_subscribed_cabinet):
    await sqlalchemy_user_repo.unsubscribe_cabinet(saved_user_with_subscribed_cabinet, _CABINET_ITEM.index)

    assert _CABINET_ITEM.index not in saved_user_with_subscribed_cabinet.group_subscribes


async def test_user_unsubscribe_cabinet_error_subscribe_not_found(sqlalchemy_user_repo, saved_user):
