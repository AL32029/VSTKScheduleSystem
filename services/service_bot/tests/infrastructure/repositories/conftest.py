...
@pytest.fixture(scope='function')
async def saved_user_without_metadata(sqlalchemy_user_repo: 'SQLAlchemyUserRepository') -> tuple['User', str]:
    user = await sqlalchemy_user_repo.save(_USER_ID)

    deleted_metadata = random.choice(list(User._REQUIRED_METADATA))

    await sqlalchemy_user_repo.session.execute(delete(UserMetadataORM).
                                               where(UserMetadataORM.user_id == _USER_ID,
