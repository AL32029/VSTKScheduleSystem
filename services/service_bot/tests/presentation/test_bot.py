...
class TestBot(AsyncBotTestMixin):
    ...
    async def test_get_group_day_schedule(self, test_container, httpx_mock):
        async with test_container(scope=Scope.REQUEST) as container:
            ...
            day_schedule_text = templater.render('day_schedule', schedule_to='group', day_schedule=_GROUP_DAY_SCHEDULE)
            ...
            httpx_mock.add_response(
                ...
                json={
                    ...
                    'error': {
                        ...
                        'detail': f'For the {_GROUP_DAY_SCHEDULE.schedule_item!s} group, there are no lessons '
    ...
    async def test_get_cabinet_day_schedule_error_schedule_not_found(self, test_container, httpx_mock):
        async with test_container(scope=Scope.REQUEST) as container:
            user_repo: SQLAlchemyUserRepository = await container.get(UserRepository)
            httpx_client = await container.get(AsyncClient)
            ...
            httpx_mock.add_response(
                ...
                json={
                    ...
                    'error': {
                        ...
                        'detail': f'For the {_CABINET_DAY_SCHEDULE.schedule_item!s} cabinet, there are no lessons '
                                  f'scheduled for tomorrow (2099-12-31)',
                        'extra': {
            ...
            assert day_schedule_error_message == f'⚠ {ScheduleForCabinetNotFound(
                _CABINET_DAY_SCHEDULE.schedule_item, 'tomorrow', datetime.date(2099, 12, 31)
            )!s}'

    async def test_get_group_day_schedule_switch_schedule_types(self, test_container, httpx_mock):
        async with test_container(scope=Scope.REQUEST) as container:
            ...
            actual_keyboard = InlineKeyboardMarkup.model_validate(last_request.reply_markup)

            day_schedule_text = text_templater.render('day_schedule', schedule_to='group',
                                                      day_schedule=_GROUP_DAY_SCHEDULE)
            day_schedule_keyboard = keyboard_templater.day_schedule(_GROUP_DAY_SCHEDULE.schedule_item.index,
                                                                    'group', 'tomorrow')
            ...
            await client.click_button(f'schedule_group_{_GROUP_DAY_SCHEDULE.schedule_item.index}_today',
                                      message=main_menu_message)
    ...
    async def test_get_group_day_schedule_error_not_found(self, test_container, httpx_mock):
        async with test_container(scope=Scope.REQUEST) as container:
            user_repo: SQLAlchemyUserRepository = await container.get(UserRepository)
            httpx_client = await container.get(AsyncClient)
            ...
            httpx_mock.add_response(
                ...
                json={
                    ...
                    'error': {
                        'code': 'GROUP_NOT_FOUND',
                        'detail': f'Group with number {_GROUP_DAY_SCHEDULE.schedule_item.index} not found',
                        'extra': {
                            'input_number': _GROUP_DAY_SCHEDULE.schedule_item.index
                        }
                    }
    ...
    async def test_get_cabinet_day_schedule_error_not_found(self, test_container, httpx_mock):
        async with test_container(scope=Scope.REQUEST) as container:
            user_repo: SQLAlchemyUserRepository = await container.get(UserRepository)
            httpx_client = await container.get(AsyncClient)
            ...
            httpx_mock.add_response(
                ...
                json={
                    ...
                    'error': {
                        'code': 'CABINET_NOT_FOUND',
                        'detail': f'Cabinet with number {_CABINET_DAY_SCHEDULE.schedule_item.index} not found',
                        'extra': {
                            'input_number': _CABINET_DAY_SCHEDULE.schedule_item.index
                        }
                    }
    ...
    async def test_get_group_day_schedule_with_deletion(self, test_container, httpx_mock):
        async with test_container(scope=Scope.REQUEST) as container:
            text_templater = await container.get(TemplateMessageRenderer)
            user_repo: SQLAlchemyUserRepository = await container.get(UserRepository)
            httpx_client = await container.get(AsyncClient)
            ...
            success_deletion_group_callback = self.client.capture.get_callback_answers()[-1].text

            assert success_deletion_group_callback == '✔ Вы перестали отслеживать группу'

    async def test_get_cabinet_day_schedule_with_deletion(self, test_container, httpx_mock):
        async with test_container(scope=Scope.REQUEST) as container:
            text_templater = await container.get(TemplateMessageRenderer)
            user_repo: SQLAlchemyUserRepository = await container.get(UserRepository)
            httpx_client = await container.get(AsyncClient)
