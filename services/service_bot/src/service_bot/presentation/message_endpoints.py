...
async def delete_message_with_delay(message: Message, request_id, update_id, user_id, message_id, delay: float = 7.5):
    ...
    logger.info('The task of deleting the message with ID %s is scheduled for %s seconds from now',
                message.message_id, delay)
    ...
    try:
        ...
        logger.warning('Error while deleting message with ID %s - %s', message.message_id, e.message)
...
async def message_add_schedule_item(message: Message, state: FSMContext, user: 'User',
    ...
    try:
        ...
        asyncio.create_task(delete_message_with_delay(error_message, request_id_var.get(), update_id_var.get(),
                                                      user_id_var.get(), message_id_var.get()))
        return
    ...
    asyncio.create_task(delete_message_with_delay(success_message, request_id_var.get(), update_id_var.get(),
                                                  user_id_var.get(), message_id_var.get()))
