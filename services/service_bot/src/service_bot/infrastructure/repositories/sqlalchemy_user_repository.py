...
class SQLAlchemyUserRepository(UserRepository):
    ...
    async def get_by_id(self, user_id: int) -> 'User':
        ...
        try:
            ...
            logger.warning('User %d metadata missing: %s', user_id, ', '.join(e.missing_keys))
    ...
    async def update_metadata(self, user: 'User', key: str, value: Any) -> None:
        ...
        try:
            ...
            logger.exception('Failed to update metadata %s for user %d', key, user.user_id)
    ...
    async def subscribe_group(self, user: 'User', group: 'Group') -> None:
        ...
        if inserted is None:
            logger.warning('User %d already subscribed to group %s', user.user_id, group.number)
    ...
    async def subscribe_cabinet(self, user: 'User', cabinet: 'Cabinet') -> None:
        ...
        if inserted is None:
            logger.warning('User %d already subscribed to cabinet %s', user.user_id, cabinet.number)
    ...
    async def unsubscribe_group(self, user: 'User', group_index: str) -> None:
        ...
        if group_index not in user.group_subscribes:
            logger.warning('User %d not subscribed to group %s', user.user_id, group_index)
            raise GroupUnsubscribeNotFound()

        stmt = (
            delete(GroupSubscribesORM)
    ...
    async def unsubscribe_cabinet(self, user: 'User', cabinet_index: str) -> None:
        """Отписка от кабинета"""
        logger.debug('Unsubscribing user %d from cabinet %s', user.user_id, cabinet_index)

        if cabinet_index not in user.cabinet_subscribes:
            logger.warning('User %d not subscribed to cabinet %s', user.user_id, cabinet_index)
            raise CabinetUnsubscribeNotFound()

        stmt = (
            delete(CabinetSubscribesORM)
            .where(CabinetSubscribesORM.user_id == user.user_id,
                   CabinetSubscribesORM.cabinet_index == cabinet_index)
        )
        await self.session.execute(stmt)

        user.cabinet_subscribes = [c for c in user.cabinet_subscribes if c != cabinet_index]
    ...
    async def _insert_default_metadata(self, user_orm: 'UserORM', keys: Iterable[str]) -> None:
        ...
        logger.info('Default metadata inserted for user %d: %s', user_orm.user_id, ', '.join(keys_to_add))
