...
class DatabaseEngineManager:
    ...
    async def rotate(self) -> bool:
        ...
        async with self._lock:
            ...
            try:
                ...
                logger.info('Checking the correctness of the connection to the database via the new engine')
                async with new_engine.connect() as conn:
                    await conn.execute(text('SELECT 1'))
                logger.info('The connection to the database via the new engine was successful')

                self._engine = new_engine

                if old_engine is not None:
                    logger.info('Closing connections to the database via the old engine')
                    asyncio.create_task(self._dispose_engine(old_engine))

                logger.info('The database credential rotation process was completed successfully')
                return True
            except (sqlalchemy.exc.SQLAlchemyError, asyncpg.exceptions.PostgresError):
                logger.exception('The database credential rotation process has completed with an error')
                return False
    ...
    async def _dispose_engine(self, engine: AsyncEngine, delay: float = 30.0) -> None:
        logger.info('The process of closing database connections is scheduled to begin in %s seconds', delay)
