...
class RedisClientManager:
    ...
    async def rotate(self) -> bool:
        ...
        async with self._lock:
            ...
            try:
                ...
                logger.info('Checking the correctness of the connection to the redis via the new engine')
                await new_client.ping()
                logger.info('The connection to the redis via the new engine was successful')
                ...
                logger.info('The redis credential rotation process was completed successfully')
                return True
            except redis.exceptions.RedisError:
                logger.exception('The redis credential rotation process has completed with an error')
                return False
    ...
    async def _close_client(self, client: Redis, delay: float = 30.0):
        logger.info('The process of closing redis connections is scheduled to begin in %s seconds', delay)
