...
async def get_group_day_schedule(group_number: str, schedule_to: Literal['today', 'tomorrow'],
                                 use_case: FromDishka['GetGroupDayScheduleUseCase']) -> 'ResponseSchema':
    schema = group_day_schedule_to_schema(await use_case.execute(group_number, schedule_to))
...
async def get_cabinet_day_schedule(cabinet_number: str, schedule_to: Literal['today', 'tomorrow'],
                                   use_case: FromDishka['GetCabinetDayScheduleUseCase']) -> 'ResponseSchema':
    schema = cabinet_day_schedule_to_schema(await use_case.execute(cabinet_number, schedule_to))
