...
class GetCabinetDayScheduleUseCase:
    ...
    async def execute(self, cabinet_number: str, schedule_to: Literal['today', 'tomorrow']) -> 'CabinetDaySchedule':
        try:
            logger.info('Obtaining the lesson schedule for cabinet %s for %s from the cache',
                        cabinet_number, schedule_to)
            day_schedule = await self.cache_repo.get_cabinet_day_schedule(cabinet_number, schedule_to)
            logger.info('The lesson schedule for cabinet %s for %s (%s) has been retrieved from the cache',
                        day_schedule.cabinet.number, str(day_schedule.date), schedule_to)
        except CacheItemNotFound:
            logger.warning('The lesson schedule for cabinet %s for %s is not found in the cache',
                           cabinet_number, schedule_to)
            ...
            logger.info('Cabinet %s was obtained from the database', cabinet_item.number)
            ...
            logger.info('The date of the lesson schedule for %s (%s) has been received',
                        schedule_to, str(schedule_date))

            logger.info('Obtaining the lesson schedule for cabinet %s for %s (%s) from the database',
                        cabinet_item.number, schedule_to, str(schedule_date))
            day_schedule = await self.schedule_repo.get_by_cabinet(cabinet_item, schedule_to, schedule_date)
            logger.info('The lesson schedule for cabinet %s for %s (%s) has been retrieved from the database',
                        cabinet_item.number, schedule_to, str(schedule_date))

            logger.info('Saving the lesson schedule for cabinet %s for %s (%s) in the cache',
                        cabinet_item.number, schedule_to, str(schedule_date))
            await self.cache_repo.set_cabinet_day_schedule(schedule_to, day_schedule)
            logger.info('The lesson schedule for cabinet %s for %s (%s) has been saved to the cache',
                        cabinet_item.number, schedule_to, str(schedule_date))
