...
class GetGroupDayScheduleUseCase:
    ...
    async def execute(self, group_number: str, schedule_to: Literal['today', 'tomorrow']) -> 'GroupDaySchedule':
        try:
            logger.info('Obtaining the lesson schedule for group %s for %s from the cache',
                        group_number, schedule_to)
            day_schedule = await self.cache_repo.get_group_day_schedule(schedule_to, group_number)
            logger.info('The lesson schedule for group %s for %s (%s) has been retrieved from the cache',
                        day_schedule.group.number, str(day_schedule.date), schedule_to)
        except CacheItemNotFound:
            logger.warning('The lesson schedule for group %s for %s is not found in the cache',
                           group_number, schedule_to)
            ...
            logger.info('Obtaining the lesson schedule for group %s for %s (%s) from the database',
                        group_item.number, schedule_to, str(schedule_date))
            day_schedule = await self.schedule_repo.get_by_group(group_item, schedule_to, schedule_date)

            logger.info('Saving the lesson schedule for group %s for %s (%s) in the cache',
                        group_item.number, schedule_to, str(schedule_date))
            await self.cache_repo.set_group_day_schedule(schedule_to, day_schedule)
            logger.info('The lesson schedule for group %s for %s (%s) has been saved to the cache',
                        group_item.number, schedule_to, str(schedule_date))
