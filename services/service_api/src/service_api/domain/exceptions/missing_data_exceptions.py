...
class GroupNotFound(NotFoundError):
    ...
    def __init__(self, group_number: str):
        ...
        extra = {"input_number": group_number}
...
class GroupDayScheduleNotFound(NotFoundError):
    ...
    def __init__(
        ...
        message = (
            f"For the {group_item!s} group, there are no lessons scheduled for "
            f"{schedule_to} ({schedule_date!s})"
        )
        extra = {
...
class CabinetDayScheduleNotFound(NotFoundError):
    ...
    def __init__(
        self,
        cabinet_item: "Cabinet",
        schedule_to: Literal["today", "tomorrow"],
        schedule_date: datetime.date,
    ):
        message = (
            f"For the {cabinet_item!s} cabinet, there are no lessons scheduled "
            f"for {schedule_to} "
            f"({schedule_date!s})"
        )
        extra = {
