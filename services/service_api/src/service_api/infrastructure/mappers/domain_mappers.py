...
def group_domain_to_orm(group: "Group") -> "GroupORM":
    return GroupORM(index=group.index, number=group.number)
...
# =================== [МАППЕРЫ LESSON] ===================
def lesson_orm_to_group_domain(
    lesson: "LessonORM", redirect: bool = False
) -> "GroupLesson":
    cabinets = [
        cabinet_orm_to_domain(cabinet)
        for cabinet in (
            lesson.cabinets_with_redirects
            if redirect
            else lesson.cabinets_without_redirects
        )
    ]

    return GroupLesson(
        start=lesson.start, end=lesson.end, name=lesson.name, cabinets=cabinets
    )
...
# =================== [МАППЕРЫ DAYSCHEDULE] ===================
def lessons_orm_to_group_day_schedule_domain(
    lessons: Iterable["LessonORM"], redirect: bool = False
) -> "GroupDaySchedule":
...
def lessons_orm_to_cabinet_day_schedule_domain(
    cabinet: "Cabinet", lessons: Iterable["LessonORM"], redirect: bool = False
) -> "CabinetDaySchedule":
    date = next((lesson.date for lesson in lessons), None)
