...
class CabinetORM(Base):
    ...
    cabinet_relationships: Mapped[list["LessonCabinetORM"]] = relationship(
        "LessonCabinetORM", back_populates="cabinet_item", lazy="selectin"
    )
...
class CabinetRedirectORM(Base):
    ...
    from_id: Mapped[str] = mapped_column(
        ForeignKey("cabinets.index", ondelete="CASCADE"), primary_key=True, index=True
    )
    to_id: Mapped[str] = mapped_column(
        ForeignKey("cabinets.index", ondelete="CASCADE"), index=True
    )
...
class LessonORM(Base):
    ...
    group: Mapped["GroupORM"] = relationship(
        "GroupORM", back_populates="lesson_relationships", lazy="joined"
    )
    ...
    def cabinets_without_redirects(self) -> list["CabinetORM"]:
        return [cabinet.cabinet_item for cabinet in self.cabinet_relationships]

    @property
    def cabinets_with_redirects(self) -> list["CabinetORM"]:
        return [
            cabinet.cabinet_item.redirected for cabinet in self.cabinet_relationships
        ]
...
class LessonCabinetORM(Base):
    ...
    cabinet_id: Mapped[str] = mapped_column(
        ForeignKey("cabinets.index", ondelete="RESTRICT"), index=True
    )
    ...
    cabinet_item: Mapped["CabinetORM"] = relationship(
        "CabinetORM", back_populates="cabinet_relationships", lazy="joined"
    )
